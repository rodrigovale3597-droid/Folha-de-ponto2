...
export const Team = ({ 
  ...
  const teams = React.useMemo(() => {
    const uniqueTeams = new Set(employees.map(e => e.team || 'Sem Equipe').filter(Boolean));
    return ['all', ...Array.from(uniqueTeams)].sort();
  }, [employees]);

  const contractTypes = React.useMemo(() => {
    const uniqueTypes = new Set(employees.map(e => e.contractType || 'Sem Regime').filter(Boolean));
  ...
  const filtered = employees.filter(e => {
    ...
    const matchesTeam = teamFilter === 'all' || (e.team || 'Sem Equipe') === teamFilter;
    const matchesContractType = contractTypeFilter === 'all' || (e.contractType || 'Sem Regime') === contractTypeFilter;
    
    return matchesSearch && matchesRole && matchesProject && matchesTeam && matchesContractType;
  });
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">Obra</label>
            <select 
              value={projectFilter} 
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">Equipe</label>
            <select 
              value={teamFilter} 
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">Regime</label>
            <select 
            ...
            >
              ...
              {contractTypes.filter(c => c !== 'all').map(type => (
      ...
      <div className="max-h-[calc(100vh-350px)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800">
        <motion.div 
          layout
            ...
            {filtered.map((emp, index) => (
              <motion.div
                ...
                transition={{ 
                  ...
                  stiffness: 400, 
                  damping: 25,
                  delay: Math.min(index * 0.05, 0.3) // Staggered entry
                }}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        {(emp.team || emp.contractType) && (
                          <div className="flex gap-2 mt-0.5">
                            {emp.team && (
                              <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">
                                {emp.team}
                              </p>
                            )}
                            {emp.contractType && (
                              <p className="text-[9px] font-bold uppercase tracking-widest text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-100 dark:border-amber-500/20">
                                {emp.contractType}
                              </p>
                            )}
                          </div>
